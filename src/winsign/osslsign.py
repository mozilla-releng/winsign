...
import construct
from winsign.asn1 import id_sha1, id_sha256, id_signedData
from winsign.pefile import certificate, pefile
...
def resign(old_sig, certs, signer):
    ...
    if new_si["digestAlgorithm"]["algorithm"] == id_sha1:
        ...
        digest_algo = "sha256"
    signer_digest = calc_signer_digest(new_si, digest_algo)
...
def write_signature(infile, outfile, signed_data):
    ci = ContentInfo()
    ci["contentType"] = id_signedData
    ci["content"] = signed_data
    sig = der_encode(ci)
...
def winsign(
    ...
    try:
        ...
        old_sig = der_decode(old_sig, ContentInfo())[0]
        old_sig = der_decode(old_sig["content"], SignedData())[0]
        certs = [cert]
        if crosscert:
...
def main(argv=None):
    ...
    if args.priv_key:
        priv_key = decode_key(open(args.priv_key, "rb").read())
